//* JavaScript ES6 Exercise

// ============================================
//* EXERCISE 1: Array Methods (push, pop, slice, splice)
// ============================================

// 1.1 Push and Pop
const fruits = ['apple', 'banana'];

// Add 'orange' to the end using push()
fruits.push('orange');

// Remove the last fruit using pop() and store it in a variable called removedFruit
const removedFruit = fruits.pop();


// 1.2 Slice vs Splice
const numbers = [1, 2, 3, 4, 5];

// Use slice() to get [2, 3, 4]
const slicedNumbers = numbers.slice(1, 4);

// Use splice() to remove 2 and 3, and insert 'two' and 'three' in their place
numbers.splice(1, 2, 'two', 'three');


// ============================================
//* EXERCISE 2: Arrow Functions
// ============================================

// Rewrite greet as an arrow function and use template literals
const greet = (name) => {
  console.log(`hello ${name}`);
};

// Create an arrow function that adds two numbers
const add = (a, b) => a + b;


// ============================================
//* EXERCISE 3: Map
// ============================================

const students = [
  { name: 'Alice', score: 85 },
  { name: 'Bob', score: 92 },
  { name: 'Charlie', score: 78 },
];

// Use map() to extract only the names
const studentNames = students.map(student => student.name);


// ============================================
//* EXERCISE 4: Filter
// ============================================

const products = [
  { name: 'Laptop', price: 1200 },
  { name: 'Mouse', price: 25 },
  { name: 'Monitor', price: 300 },
  { name: 'Keyboard', price: 80 },
];

// Filter products that cost less than $100
const affordableProducts = products.filter(product => product.price < 100);


// ============================================
//* EXERCISE 5: Reduce
// ============================================

const prices = [10, 20, 30, 40];

// Use reduce() to find largest price
const largestPrice = prices.reduce((max, current) => 
  current > max ? current : max
);


// ============================================
//* EXERCISE 6: Array Destructuring
// ============================================

const colors = ['red', 'green', 'blue'];

// Destructure to get first and second colors
const [firstColor, secondColor] = colors;


// ============================================
//* EXERCISE 7: Object Destructuring
// ============================================

const person = {
  firstName: 'John',
  lastName: 'Doe',
  age: 30,
  city: 'New York',
};

// Destructure to get firstName and age
const { firstName, age } = person;

// Destructure with renaming: firstName as 'first'
const { firstName: first } = person;

// Destructure with default values
const { country = 'USA' } = person;


// ============================================
//* CHALLENGE: Combine Everything
// ============================================

const employees = [
  { id: 1, name: 'Alice', department: 'IT', salary: 75000 },
  { id: 2, name: 'Bob', department: 'HR', salary: 65000 },
  { id: 3, name: 'Charlie', department: 'IT', salary: 80000 },
  { id: 4, name: 'Diana', department: 'Finance', salary: 70000 },
];

// Filter IT department employees
const itEmployees = employees.filter(emp => emp.department === 'IT');

// Map to get only name and salary
const itNameSalary = itEmployees.map(emp => ({
  name: emp.name,
  salary: emp.salary
}));

// Use reduce to calculate average IT salary
const totalSalary = itEmployees.reduce((sum, emp) => sum + emp.salary, 0);
const averageITSalary = totalSalary / itEmployees.length;

// Destructure the first employee's name and salary
const [{ name, salary }] = itEmployees;